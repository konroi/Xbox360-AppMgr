...
void AppManager::C_AppInstance::Relaunch()
{
	string ActivePath = AppMgr.ActiveTitle.GetAppName(true);
	BOOL bHDD1 = (strstr(ActivePath.cstr, DRIVE_HDD1) != NULL);
	BOOL bDevice = (strstr(ActivePath.cstr, DRIVE_DEVICEBASE "Package_") != NULL);

	if (bHDD1 || bDevice) {
		if (bHDD1) ActivePath.replace(ActivePath.find(DRIVE_HDD1), sizeof(DRIVE_HDD1) - 1, "HDD:\\");
		else if (bDevice) ActivePath.replace(ActivePath.find(DRIVE_DEVICEBASE), sizeof(DRIVE_DEVICEBASE) - 1, "DEVICE:\\");
		XamLoaderLaunchTitle(ActivePath.cstr, NULL);
		debug_sys("[HUD] [RelaunchTitle] -> Relaunching %s", ActivePath.cstr);
	}else if (strstr(ActivePath.cstr, "dash.xex")) XamLoaderLaunchTitle(NULL, NULL);
	else FNotify("Teapot Live - Unable to relaunch this title. Is it on your main partition?").show();
}
